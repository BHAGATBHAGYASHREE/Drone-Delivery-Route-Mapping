...
class DroneDeliveryGraph:
    ...
    def calculate_edge_cost(self, edge, weather, payload_weight, optimize_by):
        ...
        # 2. Payload Impact: Speed cap penalty and extra power draw
        ...
        # 3. Altitude climb cost: extra power draw of 2W per meter gain
    ...
    def find_shortest_path(self, start_node, end_node, weather, payload_weight, optimize_by, hazards=None):
        ...
        # Priority Queue & distances configuration
        ...
        # Algorithmic Trace to explain Dijkstra logic
        trace = []
        ...
        # Re-traverse and accumulate standard telemetry details along the shortest path
        total_distance = 0.0
        ...
        for i in range(len(path) - 1):
            ...
            # Find the edge matching path link
            found_edge = None
...
class DroneDeliveryAPIHandler(SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path):
        # API request parsing handled directly in do_GET
        ...
        # Map static web dashboard files to frontend directory
        ...
        full_path = os.path.join(frontend_dir, path_str)
        return full_path
...
def run_server():
    ...
    server_address = ('', PORT)
    httpd = HTTPServer(server_address, DroneDeliveryAPIHandler)
    print(f"\n========================================================")
    print(f"🚀 AeroRoute Pure Python Server running locally!")
    print(f"   API Gateway and Web Dashboard: http://localhost:{PORT}")
    print(f"========================================================\n")
    try:
        httpd.serve_forever()
    except KeyboardInterrupt:
        print("\nStopping Drone Delivery Gateway Server...")
        httpd.server_close()
        sys.exit(0)
